...
# List of allowed teams for validation
ALLOWED_TEAMS = [
...
# List of allowed locations for validation
ALLOWED_LOCATIONS = [
...
def display_allowed_teams():
    """
    Display a table of allowed teams.
    """
    headers = ["Teams"]
...
def display_allowed_locations():
    """
    Display a table of allowed locations.
    """
    headers = ["Locations"]
...
def add_match(filepath, new_match):
    """
    Add a new match to the JSON file.

    Parameters:
    filepath (str): Path to the JSON file.
    new_match (dict): Dictionary containing match details.
...
def get_next_match_number(matches):
    """
    Get the next match number based on the existing matches.

    Parameters:
    matches (list): List of existing matches.
...
def collect_match_details(filepath):
    ...
    Returns:
    dict: A dictionary containing the new match details, or None if an error occurs.
    """
    matches = load_games(filepath)
    if matches is None:
